...
# infile = "20210202_scene(3)_user(1)"
infile = "motion_data_20210203_GearVR_of12_fp15"

# Variables
# TRAINED_MODEL_NAME = './gearVR_realtime_model'
TRAINED_MODEL_NAME = './fafa_net2'
# Save it
# scaler_file = "gearVR_realtime_scaller.save"
scaler_file = "fafa_scaller2.save"

try:
    stored_df = pd.read_csv(str(infile)+'.csv')
    train_gyro_data = np.array(stored_df[['angular_vec_x', 'angular_vec_y', 'angular_vec_z']], dtype=np.float)
    train_bios_data = np.array(stored_df[['biosignal_0', 'biosignal_1', 'biosignal_2', 'biosignal_3', 'biosignal_4', 'biosignal_5', 'biosignal_6', 'biosignal_7']], dtype=np.float)
    ...
    proj_data = np.array(stored_df[['input_projection_left', 'input_projection_top', 'input_projection_right', 'input_projection_bottom']], dtype=np.float32)
...
#             summary,_= sess.run([merged_summary,training_op], feed_dict={x:x_train[i*window:(i+1)*window], t:t_train[i*window:(i+1)*window]})
...
quat_o = train_quat_data[DELAY_SIZE:-anticipation_size]
quat2eul_o = train_quat2eul_data[DELAY_SIZE:-anticipation_size]
...
bios_o = train_bios_data[DELAY_SIZE:-anticipation_size]
...
#                 frame_orientation[:,3]
#                 )
# 			margin = calc_optimal_overhead(input_orientation, predict_orientation, proj_data[i])
...
proj_size = (overfilling_rt[anticipation_size:,2]-overfilling_rt[anticipation_size:,0])*(overfilling_rt[anticipation_size:,1]-overfilling_rt[anticipation_size:,3])
...
raw_robust_data = np.column_stack(
	...
	bios_o[anticipation_size:],
	gyro_o[anticipation_size:]* np.pi / 180, 
	accel_o[anticipation_size:], 
	magnet_o[anticipation_size:], 
	quat_o[anticipation_size:],
	euler_o[anticipation_size:]* np.pi / 180, 
	proj_data[anticipation_size+DELAY_SIZE:-anticipation_size],
	pred_time[anticipation_size+DELAY_SIZE:-anticipation_size],
# 	eul2quat_bio(ann_pred_rt),
# 	ann_pred_rt* np.pi / 180,
	quat_o[anticipation_size:],
	euler_o[anticipation_size:]* np.pi / 180, 
...
df = pd.DataFrame(raw_robust_data,columns = ['timestamp',
							...
							'input_orientation_w', 'input_orientation_y', 'input_orientation_z', 'input_orientation_x',
							...
							'prediction_time',
							'predicted_orientation_w', 'predicted_orientation_y', 'predicted_orientation_z', 'predicted_orientation_x',
...
export_csv = df.to_csv (str(infile)+"_("+str(anticipation_time)+")_output.csv", index = None, header=True)
